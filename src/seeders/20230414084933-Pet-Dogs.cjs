...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    if (specieId) {
      const pets = []
      for (let i = 0; i < 50; i++) {
        const breedId = await queryInterface.rawSelect('PetBreed', {
          where: { specieId },
          order: queryInterface.sequelize.random()
        }, ['id'])

        const shelterId = await queryInterface.rawSelect('Shelter', {
          order: queryInterface.sequelize.random()
        }, ['id'])

        const statusId = await queryInterface.rawSelect('PetStatus', {
          order: queryInterface.sequelize.random()
        }, ['id'])
